...
class BotAI(DistanceCalculation):
    ...
    def expansion_locations(self) -> Dict[Point2, Units]:
        ...
        # - Create a graph of resource nodes
        # - Connect nearby nodes on the graph
        RESOURCE_SPREAD_THRESHOLD = 8.5
        geysers = self.vespene_geyser
        # if the minerals are offset by this amount, they would block pathing
        def is_adjacent(mineral_a, mineral_b):
            return abs(mineral_a.position.x - mineral_b.position.x) <= 2 and abs(mineral_a.position.y - mineral_b.position.y) <= 1
        ...
        for resource_a in self.resources:
            ...
            resource_graph.add_edges_from([
                (resource_a, resource_b, { 'blocking': is_adjacent(resource_a, resource_b) })
                for resource_b in self.resources
                if resource_a.distance_to(resource_b) <= RESOURCE_SPREAD_THRESHOLD
                # checking z-position eliminates nearby bases on different levels
                and abs(resource_a.position3d.z - resource_b.position3d.z) <= 0.1
            ])

        # separate into subgraphs based on which nodes are within the threshold
        resource_subgraphs = [ resource_graph.subgraph(c).copy() for c in networkx.connected_components(resource_graph) ]

        # remove mineral groups that appear only to exist for path blocking
        resource_subgraphs = [
            graph
            for graph in resource_subgraphs
            # if there are more than 8, it's probably a base
            if len(graph) > 8 or
              # if there are any geysers, it's a base
              any(node.is_vespene_geyser for node in graph.nodes()) or
              # If there aren't at least n-1 blocking edges for n nodes, it's probably a base
              len([ edge[2]['blocking'] for edge in graph.edges(data=True) ]) < len(graph) - 1
        ]

        resource_groups = [ list(graph) for graph in resource_subgraphs ]
